...
class App extends Component {
  state = {score: 0, time: 60, mainImage: imagesList[0], tab: 'FRUIT'}
  ...
  logThumbnailId = id => {
    const {score, mainImage} = this.state
    if (mainImage.id === id) {
      const newScore = score + 1
      const shuffledImagesList = [...imagesList]
      shuffledImagesList.sort(() => Math.random() - 0.5)
      this.setState({score: newScore, mainImage: shuffledImagesList[0]})
    }
  }

  onPlayAgain = () => {
    this.setState({score: 0, time: 60}, this.startTimer())
  }
  ...
  renderGamePage = () => {
    const {score, time, mainImage, logs, tab} = this.state
    console.log(logs)
    const selectedList = imagesList.filter(each => each.category === tab)
    return (
      <div className="outer-container">
        <NavBar score={score} time={time} />
        <div className="bg-container">
          <div className="container">
            <MainImage imageDetails={mainImage} />
                ...
                {tabsList.map(each => (
                  <li key={each.tabId}>
                    <button
                      type="button"
                      value={each.tabId}
                      onClick={this.changeTabSelection}
                      className={
                        each.tabId === tab ? 'selected button' : 'button'
                      }
                    >
                      {each.displayText}
                    </button>
              ...
              <ul className="thumbnails-container">
                {selectedList.map(each => (
                  <Thumbnail
                    imageDet={each}
                    key={each.id}
                    logThumbnailId={this.logThumbnailId}
                  />
                ))}
              </ul>
            </div>
